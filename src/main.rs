...
#[cfg(feature = "rp2354")]
use embassy_rp::peripherals::{DMA_CH1, FLASH, PIO0, UART0};
use embassy_rp::pio::Pio;
use embassy_rp::uart::{BufferedInterruptHandler, BufferedUart, Config as UartConfig};
#[cfg(feature = "rp2354")]
use pio_uart_dma::PioUartRxDma;
...
async fn main(spawner: Spawner) {
    ...
    // ========================================================================
    // PIO UART RX with DMA: External GNSS module input (RP2354 only)
    // GPIO4 — произвольный пин через PIO эмуляцию UART
    // DMA обеспечивает автоматическую буферизацию, предотвращая потерю пакетов
    // ========================================================================
    #[cfg(feature = "rp2354")]
    let pio_uart_rx = {
        let pio0 = Pio::new(p.PIO0, Irqs);
        ...
        // PIO UART RX с быстрым polling — программа загружается внутри конструктора
        let pio_rx = PioUartRxDma::new(
            DEFAULT_BAUDRATE,
            &mut pio_common,
            pio_sm0,
            p.DMA_CH1,  // DMA канал (зарезервирован для будущего использования)
            p.PIN_4,    // Passthrough RX from external GNSS
        );

        info!("PIO UART RX fast-poll initialized on GPIO4 at {} baud", DEFAULT_BAUDRATE);
        pio_rx
    };
    ...
    loop {
        ...
        match mode {
            ...
            OperatingMode::Passthrough | OperatingMode::PassthroughOffset => {
                ...
                // Use select3 to handle: SEC-SIGN result, GNSS data, AND generated responses (ACK, MON-VER, etc.)
                ...
                match embassy_time::with_timeout(
                    ...
                    select3(
                        ...
                        TX_CHANNEL.receive(),  // Poll responses: ACK, MON-VER, SEC-UNIQID, CFG-0x41
                    )
                ).await {
                    ...
                    Ok(Either3::Third(tx_msg)) => {
                        // TX_CHANNEL message: poll responses (ACK, MON-VER, SEC-UNIQID, CFG-0x41)
                        // These are generated by uart0_rx_task in response to drone commands
                        if let Err(e) = tx.write_all(&tx_msg).await {
                            error!("Passthrough TX_CHANNEL error: {:?}", e);
                        }
                        // Accumulate for SEC-SIGN if needed (spoofing or PassthroughOffset)
                        let current_mode = OperatingMode::load();
                        let need_accumulate = SPOOF_DETECTED.load(Ordering::Acquire)
                            || current_mode == OperatingMode::PassthroughOffset;
                        if need_accumulate {
                            let is_sec_sign = tx_msg.len() >= 4 && tx_msg[2] == 0x27 && tx_msg[3] == 0x04;
                            if !is_sec_sign {
                                let mut acc = SEC_SIGN_ACC.lock().await;
                                if let Some(ref mut accumulator) = *acc {
                                    accumulator.accumulate(&tx_msg);
                                }
                            }
                        }
                    }
...
/// PIO UART RX task - high-throughput path using fast polling
/// Processing is done in gnss_processing_task to minimize overrun risk
/// RP2354 only - uses PIO UART on GPIO4 (hardware UART1 pins not available)
///
/// Polling approach: быстрый цикл с try_read_available() вытаскивает данные
/// из FIFO как только они появляются, предотвращая переполнение.
/// FIFO = 8 слов (64µs buffer), polling каждые ~10µs = ~6x запас.
#[cfg(feature = "rp2354")]
#[embassy_executor::task]

...
pub struct UbxFrameParser {
    ...
    // Size matches GNSS_RX_CHANNEL capacity (1280)
    non_ubx_buffer: [u8; 1280],
    non_ubx_len: usize,
}
...
impl UbxFrameParser {
    ...
    pub fn feed(&mut self, byte: u8) -> Option<heapless::Vec<u8, 1280>> {
        match self.state {
    ...
    pub fn take_non_ubx_data(&mut self) -> Option<heapless::Vec<u8, 1280>> {
...
pub fn apply_offset_nav_pvt(frame: &mut [u8], off: &DynamicOffset) {
    ...
    frame[6 + 24..6 + 28].copy_from_slice(&lon.saturating_add(off.lon_1e7).to_le_bytes());
    frame[6 + 28..6 + 32].copy_from_slice(&lat.saturating_add(off.lat_1e7).to_le_bytes());
    frame[6 + 32..6 + 36].copy_from_slice(&height.saturating_add(off.alt_mm).to_le_bytes());
    frame[6 + 36..6 + 40].copy_from_slice(&h_msl.saturating_add(off.alt_mm).to_le_bytes());
}

/// Apply coordinate offset to NAV-POSLLH (0x01 0x02)
/// Payload: 28 bytes
/// Offsets: lon=4-7 (i32), lat=8-11 (i32), height=12-15 (i32), hMSL=16-19 (i32)
pub fn apply_offset_nav_posllh(frame: &mut [u8], off: &DynamicOffset) {
    ...
    frame[6 + 4..6 + 8].copy_from_slice(&lon.saturating_add(off.lon_1e7).to_le_bytes());
    frame[6 + 8..6 + 12].copy_from_slice(&lat.saturating_add(off.lat_1e7).to_le_bytes());
    frame[6 + 12..6 + 16].copy_from_slice(&height.saturating_add(off.alt_mm).to_le_bytes());

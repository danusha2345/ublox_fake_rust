...
fn test_modify_nav_pvt_spoof_replaces_coords_and_degrades_status() {
    ...
    assert_eq!(frame[6 + 23], 2, "num_sv must be 2 (spoof marker)");
}
...
fn test_modify_nav_sat_sets_num_svs_2() {
    ...
    assert_eq!(frame[6 + 5], 2, "num_svs must be 2 (spoof marker)");
}
...
fn test_full_pipeline_offset_then_spoof_replace() {
    ...
    assert_eq!(frame[6 + 23], 2, "num_sv should be 2 (spoof marker)");
}

...
Что делает feature: захват **двух независимых** буферов одновременно.

**Сторона chip → плата (UART1 RX, ~6.7 КБ):**
- Линейный one-shot буфер `[u8; 8192]`. Замораживается по условиям: буфер заполнен / 4 с прошло / встретилась подстрока `$GNRMC` (начало steady-state).
- Маркер RTT: `==RAWCAP BEGIN/END==` + `RAWCAP N off [hex…]`.
- В `Emulation` режиме захват не работает (мы сами генерируем поток).

**Сторона drone → плата (UART0 RX, ≤8 КБ):**
- Аналогичный буфер. Замораживается по: буфер заполнен / 25 с прошло / встретилась подстрока `$CFGKEY` (последняя команда стандартного handshake — у Air 3S/Mavic приходит ~через 20 с после первой команды).
- Маркер RTT: `==RAWCAP_DR BEGIN/END==` + `RAWCAP_DR N off [hex…]`.
- Активен в любом режиме (включая Emulation — там видно как наш эмулятор отвечает на drone-команды).
- Включает «мусорные» 38 байт UART-glitch префикса перед первым `$PDTINFO` (типичный артефакт включения дрона).

**Общее:**
- Задача `raw_boot_dump_task` каждые 10 с эмитит оба замороженных буфера в RTT. Цикл бесконечный — поздний `probe-rs attach` увидит дамп на следующем тике.
- Длинная пауза (>500 мс) с новыми байтами после неё → ре-арм буфера, `EPOCH++` (повторный power-cycle источника).

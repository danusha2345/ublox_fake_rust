...
- **Neo's authoritative fix feed = ExtRTCM 4074 sub 0x0FF (Receiver Information)** — 160-byte big-endian payload inside an RTCM3 frame (preamble 0xD3, msg-id 4074, sub-id 0x0FF). Carries: `Quality` byte (offset 56: 0=invalid, 1=single, 2=DGPS, 4/5=RTK, 6=INS), `SatNum` (offset 7), `Lon/Lat` (S64, units `2^-32 deg`, offsets 8/16), `Hmsl` (offset 28), all DOPs (offsets 75-83), full ECEF + accuracies + UTC time + clock state. While chip has no signal, `Quality=0` and our forced-3D NMEA gets ignored. To convince Neo of a 3D fix on Mode 0 we must either drop 4074 entirely (filter in RTCM router) **or** synthesize a 4074-0xFF frame with `Quality=1, SatNum=16, target coords`. Spec: `docs/UC6580I.md` §6.3.1, source: Unicore Firebird II Protocol Spec R1.2 (`.scratch/unicore_doc/firebird_ii_protocol.txt` §5.2.2.1). Sub `0x0FE` (Signal Information) is the supporting per-satellite tracking record and would need similar handling.
- **ExtRTCM 4074 full inventory** (`docs/UC6580I.md` §6.3, fully studied 2026-04-27): 9 sub-IDs documented in Firebird II R1.2 (0x0FF/FE/FD/FB/F9/EA/E9/E8/E6/E4 — full byte layouts captured), plus 4 **undocumented** sub-IDs observed on Neo: `0x000` (5-byte session marker `A8 5A`), `0x001` (3-byte heartbeat ping, empty payload), `0x002` (45-byte config snapshot, ~0.02 Hz, repeats `00 C8 00 01 01 08 00` entry), `0x0FC` (130-byte payload with structure `01 01 + 32 zero + 80 high-entropy + 16 zero` — looks crypto-like, possibly chip authentication/firmware fingerprint). Sensor-fusion subs `0x00B–0x014` apply only to UM621/UM681A — not emitted by UC6580. Mode 4 forwards chip's authentic `0x0FC` verbatim, which may explain why it's accepted while a fully-synthesized stream would not be.
- **4074-0xFF is cryptographically validated by drone** (live tested 2026-04-27, Neo + UC6580I): drone rejects ANY modification (in-place rewrite with recomputed CRC24Q correctly verifying via `rtcm3::verify_frame`) AND any drop of 0x0FF (post-22s drop = "module not accepted"; pre-22s modify = "module not accepted immediately"). Drone validates 4074-0xFF stream from boot, **before** $CFGKEY exchange. CFGKEY response is **constant byte-for-byte across all sessions** = static chip-model identifier, NOT per-session challenge or signing key delivery. Signing key embedded in chip firmware secure storage, not exposed via any protocol message. Drone has **separate trust models**: NMEA accepted as-is (modify works for spoofing coords in Mode 4), 4074 must be byte-perfect. **Direct 0x0FF spoof is functionally blocked at the protocol layer** without firmware-level reverse engineering of the signing key. See memory `project_unicore_4074_hmac_signature.md` for full experimental matrix and paths forward.
